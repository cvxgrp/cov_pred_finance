...
def _regularize_correlation(R, r):
    ...
    D = np.stack(
        [np.diag(np.diag(R[i, :, :] - R_lo[i, :, :])) for i in range(R.shape[0])]
    )
...
def iterated_ewma(
    ...
    if clip_at:  # Clip returns
        returns = np.clip(
            returns, returns_mean - clip_at * D, returns_mean + clip_at * D
        )
    ...
    if mean:
        ...
        means = {
            times[i]: pd.Series(means[i], index=returns.columns)
            for i in range(len(times))
        }
        covariances = {
            times[i]: pd.DataFrame(
                Sigmas[i], index=returns.columns, columns=returns.columns
            )
            for i in range(len(times))
        }
